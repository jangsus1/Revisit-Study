...
function Phase2({ parameters, setAnswer }) {
  ...
  const { coordinates, example, seconds, label_seconds, correlation, label, X, Y, type, tilt_angle } = parameters;
  ...
  // Calculate shear angle from tilt angle (angle between positive x and y axes)
  // Default to 90 degrees (normal, no tilt) if not provided
  const tiltAngleDegrees = tilt_angle !== undefined && tilt_angle !== null ? tilt_angle : 90;
  const shearAngleDegrees = 90 - tiltAngleDegrees; // Convert to shear angle
  ...
  useEffect(() => {
    ...
    // Create a group for plot content that will be sheared
    const plotGroup = svg.append('g')
      .attr('class', 'plot-content-group');
    
    // Apply shear transformation to the plot group
    // Transform origin is at bottom-left corner of plot area (where axes meet)
    const plotLeft = margin.left;
    const plotBottom = fixedSize.height - margin.bottom;
    plotGroup.attr('transform', `translate(${plotLeft}, ${plotBottom}) skewY(${-shearAngleDegrees}) translate(${-plotLeft}, ${-plotBottom})`);

    // Add proper axes with visible lines (outside the shear transform)
    ...
    // Add grid lines (5 per axis) - inside the plot group so they get sheared
    const numGridLines = 5;
    const xDomain = xScale.domain();
    const yDomain = yScale.domain();
    
    // Vertical grid lines (parallel to y-axis, will be tilted)
    // Create 5 evenly spaced grid lines
    for (let i = 1; i <= numGridLines; i++) {
      const xValue = xDomain[0] + (xDomain[1] - xDomain[0]) * (i / (numGridLines + 1));
      const xPos = xScale(xValue);
      plotGroup.append('line')
        .attr('x1', xPos)
        .attr('y1', margin.top)
        .attr('x2', xPos)
        .attr('y2', fixedSize.height - margin.bottom)
        .style('stroke', '#d3d3d3')
        .style('stroke-width', 1)
        .style('opacity', 0.5);
    }
    
    // Horizontal grid lines (parallel to x-axis, will be tilted)
    // Create 5 evenly spaced grid lines
    for (let i = 1; i <= numGridLines; i++) {
      const yValue = yDomain[0] + (yDomain[1] - yDomain[0]) * (i / (numGridLines + 1));
      const yPos = yScale(yValue);
      plotGroup.append('line')
        .attr('x1', margin.left)
        .attr('y1', yPos)
        .attr('x2', fixedSize.width - margin.right)
        .attr('y2', yPos)
        .style('stroke', '#d3d3d3')
        .style('stroke-width', 1)
        .style('opacity', 0.5);
    ...
    if (regressionData) {
      ...
      // Create a group for regression bands inside the plot group
      const regressionGroup = plotGroup.append('g')
      ...
      if (useContinuous) {
        ...
        // Use the regression group that already has clipping applied
        const bandGroup = regressionGroup;
    ...
    // Add scatter points (inside plot group so they get sheared)
  ...
  }, [view, coordinates, xScale, yScale, fixedSize, margin, X, Y, regressionData, plotWidth, plotHeight, tilt_angle, shearAngleDegrees]);
