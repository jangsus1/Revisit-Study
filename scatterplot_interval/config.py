...
# Label intervals: (label_start, label_end) in seconds
label_intervals = [(0, 2), (1, 3), (2, 4), (3, 5)]
...
def create_default_components(fail_link):
    ...
    return {
        "consent": {
            "type": "markdown",
            "path": "scatterplot_interval/assets/consent.md",
            "nextButtonText": "I agree",
            "response": []
        },
        "introduction": {
            "type": "markdown",
            "path": "scatterplot_interval/assets/introduction.md",
            "response": []
        },
        "phase1_intro": {
            "type": "markdown",
            "path": "scatterplot_interval/assets/phase1_intro.md",
            "response": []
        },
        "phase2_intro": {
            "type": "markdown",
            "path": "scatterplot_interval/assets/phase2_intro.md",
            "response": []
        },
        "phase2_examples": {
            "type": "markdown",
            "path": "scatterplot_interval/assets/phase2_examples.md",
            "response": []
        },
        "phase2_main": {
            "type": "markdown",
            "path": "scatterplot_interval/assets/phase2_main.md",
            "response": []
        },
        "phase3_intro": {
            "type": "markdown",
            "path": "scatterplot_interval/assets/phase3_intro.md",
            "response": []
        },
...
def create_phase2_components():
    ...
    that is reused for all label interval conditions to control for evaluation.
    ...
    Component naming: phase2_{label_idx}_{corr}_{exp}_{label_start}_{label_end}
    Total: 4 corrs × 12 labels × 2 exp × (4 revealed intervals + 1 unrevealed) = 480 components
    """
    ...
    for corr in [2, 4, 6, 8]:
        ...
        for label_idx, label in enumerate(labels):
            ...
            for exp in range(num_exp_variants):
                ...
                # Reuse the same scatterplot for all label interval conditions (revealed)
                for label_start, label_end in label_intervals:
                    comp_name = f"phase2_{label_idx}_{corr}_{exp}_{label_start}_{label_end}"
                    components[comp_name] = {
                        "baseComponent": "phase2",
                        "parameters": {
                            "coordinates": coordinates,  # Same coordinates for all intervals
                            "example": False,
                            "correlation": actual_correlation,  # Same correlation for all intervals
                            "label": label_text,
                            "X": x,
                            "Y": y,
                            "corr": corr,
                ...
                # Create unrevealed component (labels never show - interval starts after trial ends)
                unrevealed_start, unrevealed_end = 5, 5
                unrevealed_comp_name = f"phase2_{label_idx}_{corr}_{exp}_{unrevealed_start}_{unrevealed_end}"
                components[unrevealed_comp_name] = {
                    "baseComponent": "phase2",
                    "parameters": {
                        "coordinates": coordinates,  # Same coordinates
                        "example": False,
                        "correlation": actual_correlation,  # Same correlation
                        "label": label_text,
                        "X": x,
                        "Y": y,
                        "corr": corr,
                        ...
                        "direction": "pos",
                        "seconds": 5,
                        "label_start": unrevealed_start,
                        "label_end": unrevealed_end
                    }
                }
...
def create_phase2_example_components():
    ...
    # Use intervals for examples: (0, 2) and (1, 3)
    example_intervals = [(0, 2), (1, 3)]
    ...
    for idx, label in enumerate(example_labels):
        label_text, x, y = label
        label_start, label_end = example_intervals[idx]
        # Generate scatterplot coordinates with evenly distributed target correlation
        coordinates, actual_correlation = generate_scatterplot_data(
...
def sequence_generator(phase1_components, phase2_components, phase2_example_components, phase3_components):
    ...
    # Unrevealed condition: use interval that starts after trial ends (labels never show)
    unrevealed_interval = (5, 5)  # Starts at 5 seconds (when trial ends)
    ...
    for corr_rotation in range(len(corrs)):
        ...
        for exp_pattern in range(2):
            ...
            for label_idx in range(num_labels):
                ...
                unrevealed_start, unrevealed_end = unrevealed_interval
                unrevealed_comp = f"phase2_{label_idx}_{assigned_corr}_{exp}_{unrevealed_start}_{unrevealed_end}"
                all_trials.append(unrevealed_comp)

                # Add revealed trial group (random pick from label intervals)
                revealed_group = {
                    "id": f"label_{label_idx}",
                    "order": "random",
                    "numSamples": 1,
                    "components": [f"phase2_{label_idx}_{assigned_corr}_{exp}_{start}_{end}" 
