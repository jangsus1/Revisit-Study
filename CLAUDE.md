...
- **Fork-only platform addition.** `StimulusParams.advance?()` (`src/store/types.ts`) lets a react-component stimulus ask reVISit to go to the next step. It is a Redux flag (`advanceRequested`, set by `ReactComponentController`, consumed in `NextButton` with the same guards as Enter and dropped while Check Answer is pending). Upstream v2.4.4 does not have it; keep it small and candidate it for an upstream PR rather than extending it.
- **Scatterplot studies** (`public/scatterplot*`): each has a Python `config.py` that generates its `config.json`; edit the script, not the JSON. How to build the next one (stimulus variants, naming, sequence patterns, Next-button gating, Prolific codes) is in `public/SCATTERPLOT_STUDIES.md`.
